...
% of 1 - 100 Hz and a notch filter of 50 Hz and saves the resulting *.mat files
% This step is done before running the ICA (low frequencies need to be
% removed, otherwise the ICA will not work well).
clear all
close all
clc
...
for i = 1:length(subID)
    for j = 1:length(sess)
        ...
        fnOUT = [pn.data,subID{i},'/',subID{i},'_',sess{j},'_preICAfilt.mat'];
        fnOUTb = [pn.data,subID{i},'/',subID{i},'_',sess{j},'b_preICAfilt.mat'];
        fnOUTc = [pn.data,subID{i},'/',subID{i},'_',sess{j},'c_preICAfilt.mat'];
        fnOUT_2 = [pn.data,subID{i},'/',subID{i},'_',sess{j},'_2_preICAfilt.mat'];
        ...
        if ~exist(fnOUT)
            try
                ...
                cfg = [];
                cfg.channel = 'all';
                cfg.bpfilter = 'yes'; %apply bandpass filter between 1 - 100 Hz
                cfg.bpfreq = [1 100];
                cfg.bpfilttype = 'fir';
                cfg.bsfilter = 'yes';
                ...
                disp('Preprocessing complete, saving data...')
                save(fnOUT,'data')
            end
        else
        ...
        if ~exist(fnOUTb)
            try
                load(fnINb);
                disp(sprintf('Filtering data for subject %s, session %sb',subID{i},sess{j}))
            ...
            disp(sprintf('Subject %s, session %sb has already been filtered',subID{i},sess{j}))            
        end
